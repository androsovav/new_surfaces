...
def coordinate_descent_thicknesses(constants: dict, stack: Stack) -> Tuple[Stack, float]:
    ...
    def rebuild_prefix_suffix(M_layers: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        left  = np.tile(np.eye(2, dtype=complex)[:,:,None], (1,1,constants["n_wavelengths"]))
        right = np.tile(np.eye(2, dtype=complex)[:,:,None], (1,1,constants["n_wavelengths"]))
    ...
    for _ in range(constants["iters"]):
        ...
        for sgn in (-1, +1):
            ...
            scale = (1.0 + sgn * step)
            if constants["d_min"] is not None or constants["d_max"] is not None:
                # слой-за-слоем ограничим масштаб, где это нарушает границы
                scale_vec = np.full(num_of_layers, scale, dtype=float)
                if constants["d_min"] is not None:
                    scale_vec = np.where(thickness*scale_vec < constants["d_min"], constants["d_min"]/np.maximum(thickness, 1e-30), scale_vec)
                if constants["d_max"] is not None:
                    scale_vec = np.where(thickness*scale_vec > constants["d_max"], constants["d_max"]/np.maximum(thickness, 1e-30), scale_vec)
            else:
                scale_vec = scale
            ...
            M_changed = make_M(sphi_changed, cphi_changed, stack.q, num_of_layers, constants["n_wavelengths"])
            ...
            r, t = rt_amplitudes(constants, M_total_changed)
            R, T = RT_coeffs(constants, r, t)

            mf_layers = rms_merit_layers(constants, r, t, R, T)
        ...
        if not improved:
            step *= 0.95
            if step < constants["min_step_rel"]:
                break
