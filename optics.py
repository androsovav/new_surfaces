...
Pol = Literal["s", "p", "u"]  # u = unpolarized (avg of s/p)
...
def _cos_theta_in_layer(n_layer: complex, n_incident: complex, theta_incident: float) -> complex:
    # Константа поперечной компоненты kx ⇒ n_j*sinθ_j = n_inc*sinθ_inc
...
def _M_stack(stack: Stack, wl: float, theta_inc: float, pol: Literal["s","p"]) -> Tuple[np.ndarray, complex, complex]:
    ...
    for L in stack.layers:
        nj = _n_of(L.n, wl)
        cosj = _cos_theta_in_layer(nj, n_in, theta_inc)  # поперечная компонента сохраняется
...
def RT_single(stack: Stack, wl: float, theta_inc: float = 0.0, pol: Pol = "s") -> Tuple[float, float]:
    if pol == "u":
        ...
        # Энергетический T: Re(q_sub/q_in)*|t|^2, усредняем по поляризациям
        _, q_in_s  = _M_stack(stack, wl, theta_inc, "s")[1:]
        _, q_in_p  = _M_stack(stack, wl, theta_inc, "p")[1:]
        _, q_sub_s = _M_stack(stack, wl, theta_inc, "s")[1:]
        _, q_sub_p = _M_stack(stack, wl, theta_inc, "p")[1:]
        ...
        _, q_in = _M_stack(stack, wl, theta_inc, pol)[1:]
        _, q_sub = _M_stack(stack, wl, theta_inc, pol)[1:]
        R = np.abs(r)**2
