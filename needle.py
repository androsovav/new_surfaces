...
def _test_insert(
    stack: Stack,
    pos: Tuple[PositionKind, int],
    n_new: float,
    d_new: float,
) -> Stack:
    """Реализует вставку слоя (иглы) в позицию pos."""
    kind, idx = pos
    if kind == "interface":
...
def enumerate_positions(stack: Stack) -> List[Tuple[PositionKind, int]]:
    """
    Позиции для иглы:
      - интерфейсы: 0..N (вставка перед слоем i; 0 — у подложки, N — со стороны воздуха)
      - split: середины каждого слоя
    """
    N = len(stack.layers)
...
def discrete_excitation_map(
    ...
    """
    Дискретная P-карта: оцениваем ΔMF при вставке сверхтонкого слоя
    в точках 'interface' и 'split'. Учитывается d_min.
    """
...
def needle_step(
    ...
    """
    Один шаг: строим дискретную P-карту (интерфейсы + split),
    выбираем позицию с минимальной ΔMF, вставляем 'иглу' толщиной d_init.
    """
    ...
    info = {
        ...
        "d_mf_pred": float(dmf_best[j]),
        "base_layers": len(stack.layers),
    }
    new_stack = _test_insert(stack, pos, n_new=float(n_best[j]), d_new=d_init)
...
def needle_cycle(
    ...
    """
    Игольный цикл:
      1) ищем лучшую точку вставки (P-карта),
      2) вставляем иглу толщиной d_init,
      3) локально оптимизируем толщины (координатный спуск),
      4) логируем MF, N, TOT; проверяем критерии останова.
    """
    ...
    for step in range(1, max_steps + 1):
        ...
        # 2) вставляем иглу
        pos = positions[j]
        stack = _test_insert(stack, pos, n_new=float(n_best[j]), d_new=d_init)
        ...
        # 4) лог + проверка остановов
        N = layer_count(stack)
        TOT_nmopt = total_optical_thickness(stack, wl_ref_for_tot) / 1e-9
        rel_improv = (mf - mf_new) / max(mf, 1e-12)
        history.append({
            "step": step,
            "action": "needle+optimize",
            "pos": pos,
            "n_new": float(n_best[j]),
            "d_mf_pred": float(dmf_best[j]),
            "MF": float(mf_new),
            "MF_prev": float(mf),
            "rel_improv": float(rel_improv),
            "N": int(N),
            "TOT_nmopt": float(TOT_nmopt),
        })
        mf = mf_new
